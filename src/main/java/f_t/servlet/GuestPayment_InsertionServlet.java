...
@WebServlet("/guestpayment_insertion")
public class GuestPayment_InsertionServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

   
    public GuestPayment_InsertionServlet() {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(res>0) {
			...
			RequestDispatcher rd=request.getRequestDispatcher("GuestConfirmPayment.jsp");
		...
		RequestDispatcher rd=request.getRequestDispatcher("guestretrieve_f_t");
		rd.include(request, response);
		}
